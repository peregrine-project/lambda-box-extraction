...
@[inline] def write (s' : σ) : StateT' m σ Unit := fun _ => pure ((), s')
...
abbrev M (α : Type) := ExceptT' (StateT' Id ufData) Exn α
...
@[inline] def error {α : Type} (e : Exn) : M α := ExceptT'.error e
def run {α : Type} (x : M α) (s : ufData := ∅) : Except Exn α × ufData :=
x s
...
do let r₁ ← find n₁; let r₂ ← find n₂;
   unless (r₁ = r₂) do error .nodesNotEqual
